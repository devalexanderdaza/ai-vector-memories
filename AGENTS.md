# AI Vector Memories - AGENTS.md

## CRITICAL CONFIG

```
PROJECTS_ROOT = ~/Documents/_PROGETTI/
THIS_PROJECT  = ~/Documents/_PROGETTI/ai-vector-memories

DATABASE      = ~/.ai-vector-memories/memory.db
DEBUG_LOG     = ~/.ai-vector-memories/plugin-debug.log
OPENCODE_CFG  = ~/.config/opencode/opencode.jsonc

# v1.3.0+ Config Files
CONFIG        = ~/.ai-vector-memories/config.jsonc    # User settings (JSONC with comments)
STATE         = ~/.ai-vector-memories/state.json      # Runtime state (auto-managed)
```
...
**AI Vector Memories** - Plugin memoria persistente per OpenCode, ispirato a [PsychMem](https://github.com/muratg98/psychmem) e [true-mem](https://github.com/rizal72/true-mem) con miglioramenti:
...
**Log:** `tail -f ~/.ai-vector-memories/plugin-debug.log`

**Query memories**
sqlite3 ~/.ai-vector-memories/memory.db "SELECT classification, substr(summary,1,50) FROM memory_units WHERE status='active';"

# Delete memory
sqlite3 ~/.ai-vector-memories/memory.db "UPDATE memory_units SET status='deleted' WHERE id='...';"
```
...
- Test pulizia: eseguire manualmente `rm -rf ~/.ai-vector-memories/`
- No emoji nel codice
...
- [true-mem repo](https://github.com/rizal72/true-mem)
