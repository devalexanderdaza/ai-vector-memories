...
- **Memory & Embeddings**: `ruvector` (Vector Store), `@huggingface/transformers` (NLP Embeddings)
...
- **Vectors**: Uses `ruvector` for embedding arrays and performing similarity search (cosine similarity / Euclidean distance) to fetch relevant context on subsequent prompts.
