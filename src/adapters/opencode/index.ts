...
const WORKTREE_CACHE_FILE = join(
  homedir(),
  ".ai-vector-memories",
  ".worktree-cache",
);
...
function canExtract(): boolean {
  ...
  if (now - lastExtractionTime < MIN_EXTRACTION_INTERVAL) {
    log(
      `Skipping extraction: too soon after last extraction (${now - lastExtractionTime}ms < ${MIN_EXTRACTION_INTERVAL}ms)`,
    );
    return false;
  }
  return true;
...
/**
 * Source of truth for embeddings usage in runtime hooks.
...
function debounceMessageUpdate(
  ...
  messageDebounceTimer = setTimeout(() => {
    if (pendingMessageEvent) {
      handler(
        state,
        pendingMessageEvent.properties as Record<string, unknown> | undefined,
...
export async function createTrueMemoryPlugin(
  ...
  if (
    ...
    !worktree.startsWith("unknown-project")
  ) {
  ...
  log(
    `AI Vector Memories initialized — worktree=${worktree}, maxMemories=${config.maxMemories}`,
  );
  ...
  return {
    ...
    "chat.message": async (input, output) => {
      ...
      if (isMemoryListRequest(userText)) {
        ...
        if (memories.length > 0) {
          ...
          log(
            `Memory list request detected: injected ${memories.length} memories`,
          );
        } else {
    ...
    "tool.execute.before": async (input, output) => {
      ...
      try {
        ...
        if (memories.length > 0) {
          const wrappedContext = wrapMemories(
            memories,
            state.worktree,
            "project",
          );
          ...
          log(
            `Atomic injection: ${memories.length} memories injected for ${toolName}`,
          );
        }
      } catch (error) {
    ...
    "experimental.chat.system.transform": async (input, output) => {
      ...
      if (isValidPath(ctx.worktree) && ctx.worktree !== state.worktree) {
        log(
          `Worktree changed mid-session: ${state.worktree} -> ${ctx.worktree}`,
        );
      ...
      if (injectionMode === 0 && sessionId) {
        if (hasInjected(sessionId)) {
          log(
            `Skipping injection: already injected for session ${sessionId.slice(0, 8)}...`,
          );
          return;
        }

        // Check if this is a resumed session that already has memory context
        const shouldInject = await shouldInjectResumedSession(
          state.client,
          sessionId,
        );
        if (!shouldInject) {
      ...
      try {
        ...
        const sessionId =
          input.sessionID ?? state.currentSessionId ?? undefined;
        ...
        if (allMemories.length > 0) {
          ...
          const systemArray = Array.isArray(output.system)
            ? output.system
            : [output.system];
          ...
          log(
            `Global injection: ${allMemories.length} memories injected into system prompt [embeddings=${embeddingsEnabled}]`,
          );
        }
      } catch (error) {
    ...
    "experimental.session.compacting": async (input, output) => {
      ...
      if (state.config.opencode.injectOnCompaction) {
        ...
        if (memories.length > 0) {
          const memoryContext = formatMemoriesForInjection(
            memories,
            state.worktree,
          );
...
async function processSessionIdle(
  ...
  if (isSubAgentSession(effectiveSessionId)) {
    log(
      `Skipping extraction: sub-agent session detected (${effectiveSessionId})`,
    );
    return;
  }
  ...
  const { text: conversationText, lines: roleLines } =
    extractConversationTextWithRoles(newMessages);

  log(
    "Debug: Clean conversation text (start):",
  ...
  const hasInjectedContent = injectionMarkers.some((marker) =>
    marker.test(conversationText),
  );
  if (hasInjectedContent) {
    log(
      `WARNING: Conversation contains injection markers (safety check), extractConversationText should have filtered them out`,
    );
  ...
  log(
    `Processing ${newMessages.length} new messages, ${roleLines.length} lines with role info`,
  );
  ...
  if (signals.length > 0) {
    ...
    try {
      ...
      log(
        `Debug: Processing ${humanMessages.length} Human messages for memory extraction`,
      );
      ...
      for (const humanMsg of humanMessages) {
        ...
        log(
          `Debug: Processing Human message (${msgSignals.length} signals):`,
          text.slice(0, 100),
        );

        // Calculate base signal score (average weight of matched signals)
        const baseSignalScore =
          msgSignals.reduce((sum, s) => sum + s.weight, 0) / msgSignals.length;

        // Apply role weighting (10x for Human messages)
        const roleWeightedScore = calculateRoleWeightedScore(
          baseSignalScore,
          role,
          text,
        );
        log(
          `Debug: Role-weighted score: ${roleWeightedScore.toFixed(2)} (base: ${baseSignalScore.toFixed(2)})`,
        );
        ...
        const roleAwareContext: RoleAwareContext = {
          primaryRole: role,
          roleWeightedScore,
          hasAssistantContext: roleLines.some(
        ...
        const {
          ...
          roleValidated,
          validationReason,
        } = classifyWithRoleAwareness(text, msgSignals, roleAwareContext);
        ...
        if (isolatedContent.length > 500) {
          log(
            `Skipped memory: content too long (${isolatedContent.length} chars)`,
          );
          continue;
        }

        log(
          `Debug: Classification result: ${classification}, confidence: ${confidence.toFixed(2)}, roleValidated: ${roleValidated}, reason: ${validationReason}`,
        );

        if (classification && roleValidated) {
          // Apply three-layer defense
          const result = shouldStoreMemory(
            isolatedContent,
            classification,
            baseSignalScore,
          );
          ...
          if (result.store) {
            ...
            const userLevelClassifications = [
              "constraint",
              "preference",
              "learning",
              "procedural",
            ...
            const isUserLevel =
              userLevelClassifications.includes(classification);
            ...
            if (isUserLevel) {
              ...
              log(
                `Debug: Contextual scope detection: ${scopeDecision.isProjectScope ? "PROJECT" : "GLOBAL"} (confidence: ${scopeDecision.confidence.toFixed(2)}, reason: ${scopeDecision.reason})`,
              );
            } else {
            ...
            const shouldPromoteToLtm =
              classification !== "episodic" &&
              (isExplicitIntent ||
                autoPromoteClassifications.includes(classification));
            ...
            log(
              `Stored ${classification} memory in ${store.toUpperCase()} (confidence: ${confidence.toFixed(2)}, role: ${role}, reason: ${result.reason})`,
            );
...
async function handleSessionEnd(
  ...
  try {
    ...
    if (decayed > 0 || promoted > 0) {
      log(
        `Maintenance: decayed ${decayed} memories, promoted ${promoted} to LTM`,
      );
    }
  } catch (err) {
...
async function handleMessageUpdated(
  ...
  const info = eventProps?.info as
    | { sessionID?: string; role?: string; parts?: Part[] }
    | undefined;
  const sessionId =
    info?.sessionID ??
    (eventProps?.sessionID as string | undefined) ??
...
async function handlePostToolUse(
  ...
  const toolOutput =
    output.output && output.output.length > 2000
  ...
  for (const msg of messages) {
    ...
    for (const part of msg.parts) {
      if (part.type === "text" && "text" in part) {
        ...
        const hasInjectionMarker = injectionMarkers.some((marker) =>
          marker.test(text),
        );
...
function formatMemoriesForInjection(
  ...
  const projectLevel = memories.filter(
    (m) => !userLevelClassifications.includes(m.classification),
  );
